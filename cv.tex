...
def output_event(event):
    ...
    title_line = event['role'] + r' at ' + r'\textsc{' + event['where'] + r'}'
    lines.append(title_line)
    lines.append(when_line)

    if r'courses' in event:
        lines.append(r'\textbf{Courses} ' + r' $\bullet$ '.join(event['courses']))
    if r'projects' in event:
        line = [r'\textbf{Selected projects}', r'\begin{description}[labelindent=1cm, nosep, after*=\vspace{-\baselineskip}]']
        for title, description in event['projects'].items():
            line.append(r'\item[%s] %s' % (title, description))
        line.append(r'\end{description}')
        lines.append('\n'.join(line))
